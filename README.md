...
- `hollows_hunter_scan`: Scan running processes for process hollowing and code injection with `hollows_hunter.exe`. Supports fast targeted scanning by `process_name`, `pid`, or `recent_seconds` (processes spawned in the last N seconds) with module caching.
...
```bash
# Verify configuration via entry point
mal-mcp --check
...
Since `mal-mcp` is an MCP server communicating via JSON-RPC over `stdio`, it is invoked by an **MCP Client** or tested interactively in a web browser using the **MCP Inspector**:

#### 1. Interactive Testing via MCP Inspector (Web UI)
```bash
# Option A: Direct script invocation
npx @modelcontextprotocol/inspector python server.py

# Option B: Via installed entry point
npx @modelcontextprotocol/inspector mal-mcp
```
*(Open the URL printed in the console, e.g. `http://127.0.0.1:6274`)*

#### 2. MCP Client Configuration (Claude Desktop / Cursor / Antigravity)
...
```json
{
  "mcpServers": {
    "mal-mcp": {
      "command": "python",
      "args": [
        "C:\\Users\\Flare-VM\\Desktop\\Antigravity\\mal_mcp\\server.py"
      ],
      "env": {
        "MAL_MCP_ENABLE_DYNAMIC": "1"
      }
    }
  }
}
```
*(Or set `"command": "mal-mcp"` without args if installed via `pip install -e .`)*

---

## Security Controls & Environment Variables

`mal-mcp` implements strict fail-closed containment controls. To protect host systems from accidental malware execution, live dynamic execution and raw shell interpreters are disabled by default.

### Feature Flags

| Environment Variable | Default | Permitted Tools | Purpose |
| :--- | :--- | :--- | :--- |
| `MAL_MCP_ENABLE_DYNAMIC` | `0` (Disabled) | `behavioral_full`, `execute_with_monitoring`, `procmon_start`, `procmon_stop`, `procmon_export_csv`, `fakenet_start`, `fakenet_stop`, `x64dbg_load`, `x64dbg_attach`, `x64dbg_run_script`, `blobrunner_prepare` | Unlocks live malware process detonation, kernel ProcMon tracing, FakeNet network simulation, and debugger attachment. |
| `MAL_MCP_ENABLE_SHELL` | `0` (Disabled) | `execute_powershell`, `execute_cmd` | Unlocks raw PowerShell and CMD execution. |
| `MAL_MCP_SAMPLE_ROOT` | `C:\Samples` | All static, unpacking, and dynamic tools | Restricts target analysis targets strictly to this directory to prevent reading or executing unauthorized host files. |
| `MAL_MCP_OUTPUT_ROOT` | `C:\temp\mal_mcp` | All tools generating outputs or dumps | Confines all logs, PCAPs, memory dumps, and reports to this directory to prevent path traversal outside the analysis workspace. |

### How to Enable Dynamic Tools & Fix "Feature Disabled" Refusals

If you call a dynamic tool without the flag enabled, `mal-mcp` safely returns:
```text
...
#### 1. Permanently in your Flare-VM Sandbox (Recommended)
Run in PowerShell (persists across all sessions and reboots):
```powershell
[System.Environment]::SetEnvironmentVariable("MAL_MCP_ENABLE_DYNAMIC", "1", "User")
[System.Environment]::SetEnvironmentVariable("MAL_MCP_ENABLE_SHELL", "1", "User")
```

#### 2. In your Current Terminal Session
- **PowerShell**:
  ```powershell
  $env:MAL_MCP_ENABLE_DYNAMIC = "1"
  python server.py
  ```
- **Command Prompt (CMD)**:
  ```cmd
  set MAL_MCP_ENABLE_DYNAMIC=1
  python server.py
  ```

#### 3. In your MCP Client Configuration File (`mcp.json` / Claude Desktop)
Add an `"env"` block to the server definition (see below).
...
If you want to test and execute any of the 52 tools through an interactive web browser dashboard (MCP Inspector):
```powershell
# Enable dynamic analysis in current session
$env:MAL_MCP_ENABLE_DYNAMIC = "1"
npx @modelcontextprotocol/inspector python server.py
```
