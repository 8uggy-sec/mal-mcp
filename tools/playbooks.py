...
from .network import fakenet_start, fakenet_stop, monitor_network, is_public_ioc_ip
...
async def triage_full(file_path: str) -> str:
    ...
    if not os.path.isfile(file_path):
        return f"Target file not found: {file_path}"
...
async def execute_with_monitoring(
    ...
    try:
        ...
        # Fail-closed precheck: verify backing PML file was allocated
        ...
        # 2. Launch target directly via native Windows API (no PowerShell string interpolation)
        ...
        if job_handle and target_pid:
            assign_process_to_job(job_handle, target_pid)
        ...
        async def _do_cleanup():
            nonlocal killed, proc_summary, dropped, cleanup_status
            # Forcefully terminate target process and all child processes
            if target_pid:
                try:
                    killed = kill_process_tree(target_pid, job_handle=job_handle)
                    cleanup_status["target_terminated"] = not psutil.pid_exists(target_pid)
                except Exception:
        ...
        await asyncio.shield(_do_cleanup())
...
async def behavioral_full(
    ...
    try:
        ...
        fakenet_start_msg = await fakenet_start()
        report.append(fakenet_start_msg)
        report.append("")
        # Fail-closed check for FakeNet
        if "failed to start" in fakenet_start_msg.lower():
            report.append(f"[-] Containment Precheck Failed: FakeNet failed to start. Dynamic execution ABORTED to protect external network.")
            return "\n".join(report)
        fakenet_started = True

        # 4. Launch Target directly via native Windows API (no PowerShell string interpolation)
        ...
        if job_handle and target_pid:
            assign_process_to_job(job_handle, target_pid)
        report.append(f"Target launched with PID: {target_pid}")
        ...
        async def _do_teardown():
            nonlocal killed, fakenet_log, proc_summary, dropped_files, reg_diff, cleanup_status
            ...
            if fakenet_started:
                ...
                try:
                    fakenet_log = await fakenet_stop(start_time=start_time)
                    report.append(fakenet_log)
                    fn_running = any("fakenet" in (p.info['name'] or "").lower() for p in psutil.process_iter(['name']))
                    cleanup_status["fakenet_stopped"] = not fn_running
        ...
        await asyncio.shield(_do_teardown())
...
async def generate_ioc_report(file_path: str, output_path: str = "") -> str:
    ...
    if not output_path:
        ...
        output_path = str(TEMP_DIR / f"{stem}_ioc_report.md")
