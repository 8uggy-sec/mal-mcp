...
from ..helpers import run_command_async, run_powershell_async, launch_gui_app, safe_truncate
...
async def procmon_start(output_path: str = r"C:\temp\mal_mcp\procmon.pml") -> str:
    ...
    # Stop any running procmon first
    await run_powershell_async("Stop-Process -Name Procmon*, Procmon64* -Force -ErrorAction SilentlyContinue", timeout=15)
    ...
    res = await launch_gui_app(procmon_path, args)
...
async def procmon_stop(
    ...
    if not os.path.isfile(pml_path) or os.path.getsize(pml_path) == 0:
        await run_powershell_async("Stop-Process -Name Procmon*, Procmon64* -Force -ErrorAction SilentlyContinue", timeout=10)
    ...
    # Convert PML -> CSV asynchronously so GUI dialog does not hang console
    ...
    await launch_gui_app(procmon_path, conv_args)
    ...
    # Make sure all procmon instances are closed
    await run_powershell_async("Stop-Process -Name Procmon*, Procmon64* -Force -ErrorAction SilentlyContinue", timeout=10)
...
async def procmon_export_csv(pml_path: str, csv_path: str) -> str:
    ...
    procmon_path = resolve_tool("procmon")
    cmd = f'& "{procmon_path}" /OpenLog "{pml_path}" /SaveAs "{csv_path}" /AcceptEula'
    stdout, stderr, code = await run_powershell_async(cmd, timeout=90)
    if os.path.isfile(csv_path):
        return f"Successfully exported CSV: {csv_path} ({os.path.getsize(csv_path)} bytes)"
...
async def regshot_snapshot(action: str = "compare", workspace_dir: Optional[str] = None) -> str:
    ...
    base_dir = Path(workspace_dir) if workspace_dir else TEMP_DIR
